...
uint32_t nco_phase_init(float frequency_norm) {
    ...
    float phase_float = frequency_norm * (float)SIZE_OF_SINE_LUT * (float)8388608;
...
void nco_load_sine_to_mem(nco_t *nco) {
    ...
    audio_data_t *audio_data = (audio_data_t *)nco->audio_structure->output_stream_buffer_addr;

    for (int i = 0; i < nco->target_memory_size; i++) {
        uint32_t data = sine_lut[lut_index] >> 2;
